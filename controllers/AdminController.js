...
const login_admin = async function (req, res) {
  ...
  if (admin_arr.length == 0) {
    res
      .status(200)
      .send({ message: "El correo electrónico no existe", data: undefined });
  } else {
    //LOGIN
    ...
    bcrypt.compare(data.password, user.password, async function (error, check) {
      if (check) {
        ...
        res
          .status(200)
          .send({ message: "Las credenciales no coinciden", data: undefined });
      }
    });
  }
};
...
const agregar_etiqueta_admin = async function (req, res) {
  if (req.user) {
    try {
      ...
      res
        .status(200)
        .send({ data: undefined, message: "Etiqueta ya existente" });
    }
  } else {
    res.status(500).send({ message: "NoAccess" });
  }
...
const listar_etiquetas_producto_admin = async function (req, res) {
  if (req.user) {
    var id = req.params["id"];
    var etiquetas = await Producto_etiqueta.find({ producto: id }).populate(
      "etiqueta"
    );
